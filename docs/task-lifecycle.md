...
> **对应版本**：v0.2.1
> 本文档说明 HALF 的运行时模型：项目 / 计划 / 任务三者关系、计划生成、任务执行、结构化 handoff 流转、状态流转、`result.json` 契约、轮询机制和 Agent 可用性跟踪。以代码为事实源头；文档与代码不一致时，代码为准。
...
- **TaskHandoff**：对每条相邻依赖边自动物化出的结构化交接对象，默认先创建占位记录，后续由负责人补齐内容。
...
5. **轮询命中并 finalize**：后端轮询命中文件 → 解析 JSON（允许有限自动修复：去 markdown 围栏、尾逗号修正） → 写入 `plan_json` → 状态推进到 `completed` → 前端自动调用 `POST /api/projects/:id/plans/finalize` 把它定为 `final` 并创建 tasks，同时按依赖边自动创建占位 handoff → 自动跳转 `/projects/:id/tasks`。
...
4. **应用模版**：点击"下一步"后，前端先 `PUT /api/projects/:id` 保存 `{ goal, template_inputs }`（**不保存** `planning_mode`），然后 `POST /api/process-templates/:templateId/apply/:projectId`。后端：
   ...
   - 立刻 finalize 为 `final` plan 并创建 tasks，同时按依赖边自动创建占位 handoff
...
## 结构化 Handoff              ← 仅当前序边存在 handoff 时出现
{summary / required_inputs / artifacts / open_questions / risks_or_caveats}
...
Prompt 模板的具体填充逻辑在 `services/prompt_service.py::generate_task_prompt` 中；具体模板内容由三个 generate-prompt 端点返回，不在本文档复述。当前 prompt **不再**指示 agent 写 `usage.json`（轮询仍会读取已存在的同目录 `usage.json`，参见 4.4）。若 handoff 仍为空，prompt 会显式提示负责人和下游 agent 回退到前序目录与 `result.json`。
...
后续任务的 prompt 中会同时提供两层信息：

- 前序任务的**固定输出目录**
- 若已填写，则附带对应依赖边上的**结构化 handoff**

agent 执行任务时优先消费 handoff 中的显式语义信息；若 handoff 为空，再回退到本地 git 仓库中的前序目录和 `result.json`。HALF 不负责在前端检查"前序输出是否真实存在"——这个检查发生在 agent 自己的执行环境。
...
- `handoff.md`：结构化 handoff schema、落盘格式与关系说明
